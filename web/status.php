...
if(isset($_SESSION[$OJ_NAME.'_'.'administrator'])
    ...
    if(isset($_SESSION[$OJ_NAME.'_'.'source_browser'])){
                  ...
                  $sql="WHERE (contest_id IN (SELECT contest_id FROM contest WHERE end_time<=NOW()) or contest_id=0 or contest_id is null)  ";
    }else{
        $sql="WHERE (contest_id IN (SELECT contest_id FROM contest WHERE end_time<=NOW()) or contest_id=0 or contest_id is null) and problem_id>0   ";
    }
}
    ...
    if (!$lock || $lock_time>$row['in_date'] || $row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']) {
      if ($OJ_SIM && $row['sim']>80 && $row['sim_s_id']!=$row['s_id']) {
        ...
        if( isset($_SESSION[$OJ_NAME.'_'.'source_browser'])) {
          ...
          $view_status[$i][3] .= "<a href=comparesource.php?left=".$row['sim_s_id']."&right=".$row['solution_id']." class='label label-info' style='margin-left:1px;margin-right:1px;' target=original>".$row['sim_s_id']."(".$row['sim']."%)</a><a class='label label-default' href='userinfo.php?user=".$query_result[0]."'>".$query_result[0]." ".$query_result[1]."</a>";
        }
        else {
          $view_status[$i][3] .= "<span class='label label-info' style='margin-left:1px;margin-right:1px;'>".$row['sim_s_id']."(".$row['sim']."%)</span>";
        }
      }
      else {
        $view_status[$i][3] .= "<a href=reinfo.php?sid=".$row['solution_id']." class='".$judge_color[$row['result']]."' title='$MSG_Tips'>".$judge_result[$row['result']].$mark."</a>";
