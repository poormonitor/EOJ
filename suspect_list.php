...
if (isset($_GET['cid'])) {
	...
	$sql = "select * from (select count(distinct user_id) c,ip from solution where contest_id=? and ip not in (select ip from safe_ip) group by ip) suspect inner join (select distinct ip, user_id, in_date from solution where contest_id=? ) u on suspect.ip=u.ip and suspect.c>1 order by c desc, u.ip, in_date, user_id";
